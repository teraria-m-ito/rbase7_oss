...
var refreshScoreBoard = function() {
    console.log("*1*",this.responseText); //Check write
...
var submitScore = function() {
    ...
    console.log("*2*",time_taken);   //Check write
    var xhttp = new XMLHttpRequest();
    xhttp.addEventListener("load", getScoreBoard);
    xhttp.open("GET", "score_service?launch_id=" + launch_id + "&score=" + window.score + "&time=" + time_taken, false);
    xhttp.send();
}
...
var getScoreBoard = function() {
    ...
    xhttp.open("GET", "result_service?launch_id=" + launch_id, true);
    xhttp.send();
}
